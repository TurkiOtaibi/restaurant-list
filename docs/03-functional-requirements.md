...
| FR-007 | The system shall reject unauthenticated access to MVP data. | Must | Guests cannot view lists, places, ratings, profile, or public lists. |

## Main Navigation

| ID | Requirement | Priority | Acceptance Criteria |
| --- | --- | --- | --- |
| FR-010 | The system shall provide main navigation for My Lists, Places, and My Profile. | Must | Navigation shows `قوائمي`, `الأماكن`, and `صفحتي`; restaurant, cafe, and ice cream are filters inside Places. |
...
| ID | Requirement | Priority | Acceptance Criteria |
| --- | --- | --- | --- |
| FR-050 | The system shall allow creation of a place with name, primary type, and required subtype where applicable. | Must | A valid restaurant with subtype, cafe with subtype, or ice cream place without subtype can be created. |
| FR-051 | The system shall support `description` as reserved backend metadata. | Must | Description may exist in API/model payloads but is not exposed in the current user-facing Create Place UI. |
...
| FR-054 | The system shall expose place detail. | Must | Place detail includes name, type, subtype, average rating, rating count, current user's list membership, tried state, and rating if present. |
| FR-055 | The system shall support listing places by primary type and subtype. | Must | The Places page filters restaurants, cafes, and ice cream; restaurants and cafes support subtype filters. |
...
| ID | Requirement | Priority | Acceptance Criteria |
| --- | --- | --- | --- |
| FR-060 | The system shall display restaurant places through the Places restaurant filter. | Must | Only places with type `restaurant` are shown and restaurant subtypes can be filtered. |
| FR-061 | The system shall display cafe places through the Places cafe filter. | Must | Only places with type `cafe` are shown and cafe subtypes can be filtered. |
| FR-062 | The system shall display ice cream places through the Places ice cream filter. | Must | Only places with type `ice_cream` are shown and no subtype filter is required. |
| FR-063 | Each place row shall display name, type, subtype if applicable, average rating when available, and rating count when useful. | Must | Place rows are scannable and open Place Detail. |
...
| FR-081 | The system shall allow rating values from 1 to 10 inclusive in 0.5 increments. | Must | Values outside 1 to 10 or not aligned to a half-step are rejected. |
...
| FR-085 | The system shall use upsert behavior for repeated rating submissions. | Must | First POST returns 201; repeated POST for the same user/place updates the existing row and returns 200. |
...
| FR-089 | The system shall show rated places in the user's rating archive. | Must | `تقييماتك` is the canonical archive for tried places, rating history, and private notes. |
| FR-090 | The system shall update current-user tried indicators after rating creation or update. | Must | Rated places show Tried indicator on list, place, detail, and profile surfaces where applicable. |
...
| FR-103 | The system shall display the current user's ice cream tried count. | Must | Count matches ratings on ice cream places by the user. |
| FR-104 | The system shall display the user's rating archive with rating, private notes if present, and last updated date. | Must | Profile shows `تقييماتك`; no separate tried-places section exists. |
| FR-105 | The system shall allow editing a rating from My Profile. | Must | User can update their rating and private notes from a rating archive row. |
